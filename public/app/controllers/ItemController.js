...
app.controller('ItemController', function(dataFactory,$scope,$http){
  ...
  $scope.saveEdit = function(){
    dataFactory.httpRequest('items/'+$scope.form._id,'PUT',{},$scope.form).then(function(data) {
      	$(".modal").modal("hide");
        $scope.data = apiModifyTable($scope.data,data._id,data);
    });
  }
  ...
  $scope.remove = function(item,index){
    var result = confirm("Are you sure delete this item?");
   	if (result) {
      dataFactory.httpRequest('items/'+item._id,'DELETE').then(function(data) {
